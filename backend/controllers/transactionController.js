...
export async function createTransaction(req, res) {
  try {
    const { type, amount, merchant, category, source, account, rawSms, date } = req.body;

    if (!type || amount === undefined || !merchant) {
      return res.status(400).json({ error: 'Please provide type, amount, and merchant' });
    }
...
// @desc    Batch create transactions (PhonePe / Bank SMS sync with duplicate protection)
...
// @desc    Get purely dynamic aggregated analytics from MongoDB
...
export async function getAnalytics(req, res) {
  try {
    ...
    const avgDailySpend = dayCount > 0 ? Math.round(totalExpense / dayCount) : 0;
