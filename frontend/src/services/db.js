...
// IndexedDB Local-First Database (Dexie.js)
export const db = new Dexie("PersonalExpenseDB");

db.version(1).stores({
	transactions:
		"++id, amount, type, merchant, category, date, source, account",
});

export async function saveTransaction(txn) {
	const category =
		txn.category || categorizeMerchant(txn.merchant, txn.rawSms || "");
	const entry = {
		...txn,
		category,
	...
	// Save to local IndexedDB (instant offline-first access)
	const localId = await db.transactions.add(entry);

	// Sync to MongoDB Atlas backend API if online
	try {
		...
		console.warn(
			"Backend offline, queued locally in IndexedDB:",
			err.message,
		);
	}
