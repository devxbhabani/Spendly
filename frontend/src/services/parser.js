/**
 * Enhanced Bank, UPI, and PhonePe SMS Parser
 * Extracts transaction amount, type, merchant/beneficiary, category, bank/account, and date
 * Supports all Indian Banks (UCO Bank, SBI, HDFC, ICICI, Axis, PNB, etc.) and UPI apps (PhonePe, GPay, Paytm).
 */
...
export function parseTransactionSMS(smsText) {
  ...
  const patternMatch = cleanText.match(/(?:to\s+(?:vpa\s+)?|at\s+|info[\s*:]+|beneficiary\s+|for\s+|by\s+)([a-zA-Z0-9.\-_@ ]{2,30}?)(?:\.|\s+on|\s+ref|\s+via|\s+avl|\s+bal|\s+upi|\s+report|$)/i);
  ...
  if (ddmmyyyyMatch) {
    ...
    const textDateMatch = cleanText.match(/(\d{1,2}-[a-zA-Z]{3}(?:-\d{2,4})?)/i);
    if (textDateMatch) {
